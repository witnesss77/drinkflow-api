...
@router.post("")
async def set_stocks(request: CreateStock, service = Depends(get_stocks_service), user = Depends(admin_check)):
    return await service.set_stocks(request)

@router.patch("/{stock_id:int}")
async def update_stock(stock_id: int, request: UpdateStock, service = Depends(get_stocks_service), user = Depends(admin_check)):
    return await service.update_stocks(request, stock_id)
    
@router.delete("/{stock_id:int}")
async def delete_stock(stock_id: int, service = Depends(get_stocks_service), user = Depends(admin_check)):
